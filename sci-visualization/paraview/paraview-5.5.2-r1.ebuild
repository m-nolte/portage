...
KEYWORDS="~amd64 ~x86"
SLOT="0"
IUSE="boost cg coprocessing development doc examples ffmpeg mpi mysql nvcontrol openmp plugins python +qt5 sqlite tcl test tk +webkit"
RESTRICT="test"
...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-5.5.2-include-qabstractitemview.patch
	"${FILESDIR}"/${PN}-5.5.2-include-qheaderview.patch
	"${FILESDIR}"/${PN}-5.5.2-include-qstyle.patch
	"${FILESDIR}"/${PN}-5.5.2-include-qdoublevalidator.patch
)
