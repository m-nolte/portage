...
SRC_URI="https://gitlab.dune-project.org/extensions/${PN}/repository/archive.tar.gz?ref=v2.6.0rc2 -> ${P}.tar.gz"

LICENSE="GPL-2"
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="alberta doc gmp metis mpi parmetis ug vc zlib"
...
src_configure() {
  ...
  local mycmakeargs=(
      ...
      -DCMAKE_DISABLE_FIND_PACKAGE_DLMalloc="FALSE"
      ...
      -DCMAKE_DISABLE_FIND_PACKAGE_SIONlib="FALSE"
	  -DCMAKE_DISABLE_FIND_PACKAGE_Vc="$(usex vc FALSE TRUE)"
	  -DCMAKE_DISABLE_FIND_PACKAGE_ZLIB="$(usex zlib FALSE TRUE)"
      -DCMAKE_DISABLE_FIND_PACKAGE_ZOLTAN="FALSE"
    )
  if use ug ; then
	local mycmakeargs+=(
