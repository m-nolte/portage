...
BACKPORTS="f11c5aba6ed1e88c22f1b3b6828fc085e1699b97"
MY_P=${P%_p*}
MY_PV=${PV%_p*}
...
IUSE="alsa altivec autostart bluray cec crystalhd debug dvb dvd egl fftw +hls \
	...
	vaapi vdpau +vorbis +wrapper +X +xml xmltv +xvid zeroconf ${IUSE_INPUT_DEVICES} ${IUSE_VIDEO_CARDS}"
...
src_configure() {
	...
	myconf="${myconf} --disable-hdhomerun"
	myconf="${myconf} --disable-vbox"
	myconf="${myconf} --disable-ceton"

	# Video
	myconf="${myconf} $(use_enable vdpau)"
	myconf="${myconf} $(use_enable vaapi)"
	myconf="${myconf} $(use_enable crystalhd)"
	myconf="${myconf} $(use_enable video_cards_nvidia xnvctrl)"
